...
def get_batch(x, y, word2id, noisy=False, min_len=5):
    ...
    for sent in x:
        # "word2id[w] if w in word2id, else unk" for every word in the sentence.
        # effectively converts sentence to input id array.
...
# enc_inputs = the entire input x.0 corpus, where each sentence is a id list.
# dec_inputs = actually, the entire input too but with different paddings.
# labels = literally the unmodified target corpus x.1
